...
//    lastReceivedDDPacket.ddOptions.M_More = true;
    ...
    inactivityTimer = parentArea->getProcess()->getMessageHandler()->createTimer(OSPFV3_NEIGHBOR_INACTIVITY_TIMER,this);
    pollTimer = parentArea->getProcess()->getMessageHandler()->createTimer(OSPFV3_NEIGHBOR_POLL_TIMER,this);
    ddRetransmissionTimer = parentArea->getProcess()->getMessageHandler()->createTimer(OSPFV3_NEIGHBOR_DD_RETRANSMISSION_TIMER,this);
    updateRetransmissionTimer = parentArea->getProcess()->getMessageHandler()->createTimer(OSPFV3_NEIGHBOR_UPDATE_RETRANSMISSION_TIMER,this);
    requestRetransmissionTimer = parentArea->getProcess()->getMessageHandler()->createTimer(OSPFV3_NEIGHBOR_REQUEST_RETRANSMISSION_TIMER,this);
    ...
    updateRetransmissionTimer->setName("OSPFv3Neighbor::NeighborUpdateRetransmissionTimer");
    requestRetransmissionTimer->setName("OSPFv3Neighbor::NeighborRequestRetransmissionTimer");
    state = new OSPFv3NeighborState;
//    previousState = NULL;
}
...
void OSPFv3Neighbor::processEvent(OSPFv3Neighbor::NeighborEventType event)
{
...
*/
    parentInterface->getArea()->getProcess()->getMessageHandler()->clearTimer(ddRetransmissionTimer);
...
*/
OSPFv3Neighbor::NeighborStateType OSPFv3Neighbor::getState() const
{
    return (OSPFv3Neighbor::NeighborStateType)state->getState();
}

const char* OSPFv3Neighbor::getStateString(OSPFv3Neighbor::NeighborStateType stateType)
{
    switch (stateType) {
        case STATE_DOWN:           return "Down";
        case STATE_ATTEMPT:        return "Attempt";
        case STATE_INIT:           return "Init";
        case STATE_TWOWAY:         return "TwoWay";
        case STATE_EXCHANGE_START: return "ExchangeStart";
        case STATE_EXCHANGE:       return "Exchange";
...
void OSPF::Neighbor::removeFromRequestList(OSPF::LSAKeyType lsaKey)
{
    ...
    if ((getState() == OSPF::Neighbor::STATE_LOADING) && (linkStateRequestList.empty())) {
        clearRequestRetransmissionTimer();
